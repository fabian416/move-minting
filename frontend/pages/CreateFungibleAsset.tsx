...
// ⚠️ QUITAMOS el import estático de Irys para poder bypassear
...
// ========= BYPASS FLAGS =========
const SKIP_IRYS = import.meta.env.VITE_SKIP_IRYS === "true";
// Puedes cambiar el placeholder si quieres:
const PLACEHOLDER_ICON =
  import.meta.env.VITE_PLACEHOLDER_ICON ||
  "https://placehold.co/512x512/png?text=Asset";
...
export function CreateFungibleAsset() {
  ...
  const disableCreateAssetButton =
    ...
    !account ||
    isUploading ||
    (!SKIP_IRYS && !image); // 👈 solo exigimos imagen cuando NO hay bypass
  ...
  const onCreateAsset = async () => {
    try {
      ...
      let iconURL: string;
      if (SKIP_IRYS) {
        // En dev, usamos un placeholder y no subimos nada
        iconURL = PLACEHOLDER_ICON;
      } else {
        // En prod (o cuando quieras testear Irys), sí exigimos imagen
        if (!image) throw new Error("Select image first");

        // Carga perezosa para no romper en dev si borras utils/Irys
        const { checkIfFund, uploadFile } = await import("@/utils/Irys");
        ...
        // Upload a Irys
        iconURL = await uploadFile(aptosWallet, image);
      }
      ...
      const response = await signAndSubmitTransaction(
        createAsset({
          ...
          iconURL, // 👈 pasa el placeholder o el URL real de Irys
          projectURL,
          mintFeePerFA,
          mintForMyself,
          maxMintPerAccount,
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row items-start justify-between px-4 py-2 gap-4 max-w-screen-xl mx-auto">
        <div className="w-full md:w-2/3 flex flex-col gap-y-4 order-2 md:order-1">
          ...
          {/* Solo mostramos el uploader cuando NO estamos en bypass */}
          {!SKIP_IRYS && (
            <Card>
              <CardHeader>
          ...
          <ConfirmButton
            ...
            confirmMessage={
              SKIP_IRYS ? (
                <p>In dev, image upload will be skipped and a placeholder icon will be used.</p>
              ) : (
                <>
                  <p>
                    The upload process requires at least 1 message signature to upload the asset image file into Irys.
                  </p>
                  <p>In case we need to fund an Irys node, a transfer transaction submission is required as well.</p>
                </>
              )
            }
          />
